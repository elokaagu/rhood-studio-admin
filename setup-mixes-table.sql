...
-- Create storage policies for mixes bucket
CREATE POLICY "Public read access for mixes" ON storage.objects
  FOR SELECT USING (bucket_id = 'mixes');

CREATE POLICY "Authenticated users can upload mixes" ON storage.objects
  FOR INSERT WITH CHECK (bucket_id = 'mixes' AND auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can update mixes" ON storage.objects
  FOR UPDATE USING (bucket_id = 'mixes' AND auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can delete mixes" ON storage.objects
